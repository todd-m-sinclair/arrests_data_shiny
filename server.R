function(input, output, session){
  ...
  # Render the box header for top 5 crime rate  
  output$head1 <- renderText(
    paste("5 states with high rate of", input$var2, "Arrests")
  )
  
  # Render the box header for low 5 crime rate  
  ...
  # Render table with 5 states with low arrests for specific crime type
  ...
  output$map_plot <- renderPlot({
    new_join %>% 
      ...
      theme_void() +
      labs(title = paste("Choropleth map of", 
                         input$crimetype , 
